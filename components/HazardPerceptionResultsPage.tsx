  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 flex items-center justify-center min-h-screen">
      <div className="max-w-xl w-full text-center bg-white dark:bg-slate-800/50 p-8 rounded-2xl border border-gray-200 dark:border-slate-700 animate-fadeInUp shadow-2xl shadow-gray-200/50 dark:shadow-none">
        <h1 className={`text-5xl font-bold ${passed ? 'text-teal-500 dark:text-teal-400' : 'text-red-500'} tracking-tight`}>
          {passed ? 'PASS' : 'FAIL'}
        </h1>
        <p className="mt-4 text-gray-600 dark:text-gray-300 text-lg">Your Hazard Perception Score</p>
        <p className="text-6xl font-bold my-2 text-gray-900 dark:text-white">{totalScore}<span className="text-4xl text-gray-400">/{maxScore}</span></p>
        ...
        <div className="text-left bg-gray-100 dark:bg-slate-900/50 p-4 rounded-lg border border-gray-200 dark:border-slate-700 mb-10">
            <h3 className="font-semibold text-lg mb-3 text-gray-900 dark:text-white">Score Breakdown</h3>
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                {scores.map((score, index) => (
                    <div key={index} className="flex justify-between items-center text-gray-600 dark:text-gray-300">
                        <span className="text-base">Clip {index + 1}:</span>
                        <span className={`font-semibold text-base ${score > 0 ? 'text-teal-500 dark:text-teal-400' : 'text-red-500'}`}>{score} points</span>
                    </div>
                ))}
            </div>
        </div>
